...
#define SA818_TX_PIN 14  // Serial1 TX für SA818 an GPIO14
#define SA818_RX_PIN 15  // Serial1 RX für SA818 an GPIO15
#define AUDIO_OUT_PIN 16 // Audioausgang (LEDC) an GPIO16
...
void sampleHandler(void *p) {
  ...
  while (1) {
    if (TFLAG) {
      ...
      ledc_set_duty(LEDC_LOW_SPEED_MODE, LEDC_CHANNEL_1, v);
...
void sendMorse(const char* message, int wpm) {
  ...
  ledc_set_freq(LEDC_LOW_SPEED_MODE, LEDC_TIMER_0, 1900);
...
void setup() {
  ...
  hw_timer_t* timer = timerBegin(8000000); // Frequenz direkt angeben
  timerAttachInterrupt(timer, &audioISR);
  timerAlarm(timer, 8000000 / F_SAMPLE, true, 0); // Alarm aktivieren
  ...
  ledc_channel.duty = 2;
  ledc_channel.gpio_num = AUDIO_OUT_PIN;
